...
class Adminroles_model extends CI_Model
{
    ...
    public function post_role($title,$resource_list)
	{
		...
		$this->db->insert('pm_authorization_rule', $data);
	...
	{
		 $sql1="SELECT 	* FROM  `pm_authorization_rule` ";
	...
	{
		  $sql1="delete FROM  `pm_authorization_rule` where role_id=".$id;
		$query1= $this->db->query($sql1);
	   return 1;
	}
	public function edit_role($id)
	{
		  $sql1="select * FROM  `pm_authorization_rule` where role_id=".$id;
		$query1= $this->db->query($sql1);
	   return $query_res1=$query1->result();
	}
	public function update_role($role_id,$title,$resource_list)
	{
        ...
        $this->db->update('pm_authorization_rule', $data);
		return 1;
	}
